...
docker exec -i "$DB_CONTAINER" mariadb nextcloud -u root --password="$HABIDAT_NEXTCLOUD_DB_ROOT_PASSWORD" < "$STAGING_DIR/db.sql"
...
# maintenance:mode --off below needs the instance up, and a restored database can
# trigger an upgrade on first start.
