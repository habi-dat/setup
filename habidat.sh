...
update_installed_modules() {
	...
	then
		...
		sed -i '/HABIDAT_USER_INSTALLED_MODULES/d' store/auth/auth.env
		echo "HABIDAT_USER_INSTALLED_MODULES=$HABIDAT_USER_INSTALLED_MODULES" >> store/auth/auth.env
...
update_module() {
	...
	then
		...
		then
			...
			cp "$1/version" "store/$1/version"
			# cd .. # handled by subshell
			print_done
		fi
	else
...
print_admin_credentials() {
	...
	then
		...
		prefix "habi*DAT admin credentials: username is ${bold}${HABIDAT_ADMIN_EMAIL:-not set}${reset}, password is ${bold}${HABIDAT_ADMIN_PASSWORD:-unknown}${reset}"
	fi
}
